...
for i, q in enumerate(quiz):
    file = f"slide_{i}.png"
    imgkit.from_string(create_html(q, i), file, config=config)
    images.append(file)

# Answer slide
answer_html = "<h1>Answers</h1>"
for i, q in enumerate(quiz):
    answer_html += f"<h3>Q{i+1}: {q['answer']}</h3>"

imgkit.from_string(answer_html, "answer.png", config=config)
